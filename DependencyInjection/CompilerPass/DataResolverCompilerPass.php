...
class DataResolverCompilerPass implements CompilerPassInterface
{
     ...
     */
    private const DATA_RESOLVER_TAG = 'zitec_autocomplete_data_resolver';

    /**
     * The id of the data resolver manager service.
     */
    private const DATA_RESOLVER_MANAGER_ID = 'zitec.form_autocomplete.data_resolver_manager';

    /**
     * The name of the parameter containing the global suggestions limit.
     */
    private const SUGGESTIONS_LIMIT_PARAMETER = 'zitec.form_autocomplete.suggestions_limit';
     ...
     * @throws LogicException
     */
    protected function validateDataResolverClass(ContainerBuilder $container, string $serviceId)
    {
     ...
     * @param array $tags
     */
    protected function configureSuggestionsLimit(ContainerBuilder $container, string $serviceId, array $tags)
    {
        $definition = $container->getDefinition($serviceId);
        $class = $container->getParameterBag()->resolveValue($definition->getClass());
        $reflection = new \ReflectionClass($class);
